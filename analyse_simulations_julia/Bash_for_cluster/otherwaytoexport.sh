...
REMOTE_BASE_PATH="/mnt/home/rbeauvallet/ceph/dataHeisenberg/ParallelDisorder/test"
LOCAL_DEST="../DATA_Cluster/Disorder_parallel/test"  # Répertoire local de destination

# Créer dossier local s'il n'existe pas
mkdir -p "$LOCAL_DEST"

# Récupérer la liste des fichiers output_ à copier
ssh ${REMOTE_USER}@${REMOTE_HOST} "find ${REMOTE_BASE_PATH}/h_* -type f -name 'mps_*'" > remote_output_files.txt
